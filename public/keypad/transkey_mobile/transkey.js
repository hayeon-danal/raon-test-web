...
var setNumberMobileHash =
  "5915c8880160c9405210946821fd475211fd0a19366aafea618b8740864b1726"; //numberMobileFX_ND
...
function mTranskeyObj(inputObj, width, div, keyType, keyboardType, dataType) {
  ...
  this.allocation = function () {
    ...
    try {
      ...
      tk_alert(
        "11[mTranskey Error] : Cannot load TransKey. Network is not available."
      );
      return false;
    }
  };
          ...
          if (request.readyState == 4 && request.status == 200) {
            ...
            quertyImg.onload = function () {
              ...
              upper_request.onreadystatechange = function () {
                if (
                  upper_request.readyState == 4 &&
                  upper_request.status == 200
                ) {
                  var upper_imageUrl = convertBinarytoFile(
                    upper_request.response
                  );
              ...
              special_request.onreadystatechange = function () {
                if (
                  special_request.readyState == 4 &&
                  special_request.status == 200
                ) {
                  var special_imageUrl = convertBinarytoFile(
                    special_request.response
                  );
                  self.specialDiv.style.backgroundImage =
  ...
  this.getDummy = function () {
    ...
    try {
      ...
      tk_alert(
        "22[mTranskey Error] : Cannot load TransKey. Network is not available."
      );
      return false;
    }
  };
  ...
  this.setKeyIndex = function (inputObj) {
    ...
    try {
      ...
      tk_alert(
        "33[mTranskey Error] : Cannot load TransKey. Network is not available."
      );
      return false;
    }
  };
  ...
  this.getInitTime = function () {
    ...
    try {
      ...
      tk_alert(
        "44[mTranskey Error] : Cannot load TransKey. Network is not available."
      );
      return false;
    }
  ...
  this.setBgImageForDivType1UseBlob = function (type) {
    ...
    request.open(
      "GET",
      getUrl("getKey", this, type, this.allocationIndex) +
        "&talkBack=" +
        self.talkBack,
      true
    );
      ...
      if (request.readyState == 4 && request.status == 200) {
        ...
        if (type == "single") {
          ...
          for (var i = 1; 5 > i; i++) {
            for (var j = 0; 11 > j; j++) {
              try {
                ...
                console.log(
                  "[mTranskey Error] : backgroundImage error , (type=" +
                    type +
                    ")"
                );
              }
          ...
          for (var j = 0; k > j; j++) {
            try {
              if (relocation == true && j == 1) {
                self.div.childNodes[5].childNodes[
                  j
                ].childNodes[0].style.backgroundImage =
                  "url('" + transkey_url + "/images/re_btn.png')";
              } else {
...
function mTranskey() {
  ...
  if (
    useBalloon &&
    this.pcFilter.indexOf(navigator.platform.toLowerCase()) < 0
  ) {
  ...
  this.getPublicKey = function (url) {
    ...
    try {
      ...
      tk_alert(
        "55[mTranskey Error] : Cannot load TransKey. Network is not available."
      );
      return false;
    }
  };
  ...
  this.generateSessionKey = function (url) {
    ...
    if (useGenKey) {
      ...
      if (cert_pub.indexOf("Error") > -1) {
        tk_alert(
          "66[mTranskey Error] : Cannot load TransKey. Network is not available."
        );
        return false;
      }
      ...
      if (request.readyState == 4 && request.status == 200) {
        ...
        if (request.responseXML) {
              ...
              for (var j = 0; j < node.childNodes.length; j++) {
                if (node.childNodes[j].nodeValue.length > 10) {
                  if (useCSP) {
                    ...
                    mtk.licExpiredDate = getLicExpiredDate(
              ...
              return new Promise((resolve, reject) => {
                setTimeout(() => {
                  ...
                  if (useSession) {
                    ...
                    transkey[inputs.item(index).id].setKeyIndex(
                      inputs.item(index)
                    );
                  }
                }, 0);
          ...
          if (useCSP) {
            ...
            if (
              "win16|win32|win64|mac|macintel".indexOf(
    ...
    try {
      ...
      tk_alert(
        "88[mTranskey Error] : Cannot load TransKey. Network is not available."
      );
      return false;
    }
  };
  ...
  this.resetToken = function (url) {
    ...
    try {
      ...
      tk_alert(
        "99[mTranskey Error] : Cannot load TransKey. Network is not available."
      );
      return false;
    }
  };
  ...
  this.resetSessionKey = function (url) {
    ...
    try {
      ...
      tk_alert(
        "10[mTranskey Error] : Cannot load TransKey. Network is not available."
      );
      return false;
    }
  };
  ...
  this.setHiddenField = function (inputObj, ExE2E) {
    ...
    if (!useSession) {
      ...
      var sessionKey = document.getElementById(
  ...
  this.getText = function (encrypted, ele) {
      ...
      if (request.readyState == 4 && request.status == 200) {
        if (request.responseText.indexOf("SessionError") > -1) {
          ...
          mtk.now.inputObj.value =
            mtk.now.inputObj.value + request.responseText;
          if (mtk.now.useInput) {
    ...
    try {
      ...
      tk_alert(
        "12[mTranskey Error] : Cannot load TransKey. Network is not available."
      );
      return false;
    }
  };
  ...
  this.patternTouchOver = function (b) {
    ...
    if (mtk.now.isdrawing) {
      ...
      if (thisbutton != to) {
        ...
        if (thisline) {
          if (
            b.firstElementChild.className.indexOf("tk_pattern_btn_touched") < 0
          ) {
            b.firstElementChild.className =
              "tk_pattern_btn tk_pattern_btn_touched";
  ...
  this.processTouchMove = function (x, y) {
    ...
    if (!mtk.now.gridsize) {
      ...
      mtk.now.gridsize =
        parseInt(mtk.now.buttons[1].pos.left) -
        parseInt(mtk.now.buttons[0].pos.left);
    }
    ...
    if (buttonnr != to) {
      ...
      if (
        distancex < mtk.now.gridsize / 2 &&
        distancey < mtk.now.gridsize / 2
      ) {
...
mTranskey.prototype.setCapStyle = function () {
  ...
  if (transkey_divType == 1 && mtk.now.keyboardType == "qwertyMobile") {
    if (mtk.now.keyType == "upper") {
      ...
      mtk.now.div.childNodes[4].childNodes[0].classList.add(
        "dv_" + cssName + "_div_c"
      );
    } else {
      mtk.now.div.childNodes[4].childNodes[0].classList.remove(
      ...
      if (i == 5) {
        ...
        if (key_margin > 0) {
          ...
          transkeyObj.div.childNodes[i].childNodes[9].style.border =
            "1px solid rgb(217, 218, 226)";
          transkeyObj.div.childNodes[i].childNodes[9].style.borderRight =
            "0px;";
    ...
    if (mtk.now.useInput) {
      ...
      if (transkey_divType == 1) {
        ...
        for (var i = 0; i < div.childNodes[1].childNodes.length; i++) {
          totalWidth +=
            div.childNodes[1].childNodes[i].offsetWidth + key_margin;
        }
...
function tk_alertLayerResize() {
  ...
  for (var i = 0; i <= alertCnt; i++) {
    ...
    if (div2) {
      div2.style.top =
        window.innerHeight / 2 - div2.children[0].clientHeight / 2 + "px";
      div2.style.left =
        window.innerWidth / 2 - div2.children[0].clientWidth / 2 + "px";
    }
  }
}
