...
// var transkey_url = '/keypad/transkey_mobile';


// FE LOCAL SURL, APIURL
// var transkey_surl = 'http://192.168.210.133:20288/transkeyServlet';
// var transkey_apiurl = 'http://192.168.210.133:20288/transkey/api/';

// DEV SURL, APIURL
// var transkey_surl =
//   'https://test-damoum.danalpay.com/raon-keyboard/transkeyServlet';
// var transkey_apiurl =
//   'https://test-damoum.danalpay.com/raon-keyboard/transkey/api/';

// PRODUCTION SURL, APIURL
...
var transkey_apiuse = true; //default : false | use : true | not use : false
...
var useSha2 = true; //4.6.14.9 이하 모듈 : false

//cors
var useCORS = true;
//var tk_origin = '&origin=0'; // local
var tk_origin = '&origin=1'; // dev
// var tk_origin = '&origin=2'; // production
...
//local 
var setQwertyMobileHash = "83a8883e6b1a6365997611d06ae3e16bd97bc1d46a74742645661883611feaab"; //qwertyMobileFX_space
var setNumberMobileHash = "ee9ee9f05ef9eebb9e9784433279c83c7c1cf247a2e98370ecabc87e2c2d0a5f"; //numberMobileFX

// 7469a836815bb6cb556ecdafb8e45a3c0618f7233f0767fd433ab924cc4caab9
// 7fb50fb0e8cd5f4d2b5f5f6263bfb667c9cf8075227309a2edd608ba6662af04

// dev 
// var setQwertyMobileHash = "bab650daf5d4de7a1d4111c324d2a15523bb50476573449d2cb112156de72cc7"; //qwertyMobile
// var setNumberMobileHash = "73ff3f5caa85700b7b4b37e4d9fde2ca596b2bd1fae0805494d0c82f6d8eb67b"; //numberMobile
var _useBioAuth = false;
...
if (useSha2) {
  document.write(
    '<script type="text/javascript" src="' +
      transkey_url +
      '/jsbn/jsbn-sha2-min.js"></script>'
  );
} else {
  document.write(
    '<script type="text/javascript" src="' + transkey_url + '/jsbn/jsbn-min.js"></script>'
  );
}

document.write(
  '<script type="text/javascript" src="' + transkey_url + '/typedarray.js"></script>'
);
...
function initmTranskey(useBioAuth) {
  _useBioAuth = useBioAuth;
  setMaxDigits(131);
  // console.log("init useSession::: ", useSession)
...
function mTranskeyObj(inputObj, width, div, keyType, keyboardType, dataType) {
    ...
    if (this.readyState == 4 && this.status == 200) {
      if (this.responseText.indexOf('SessionError') > -1) {
        ...
        if (errCodes[1] == '1') {
          ...
          tk_alert('allocationCallback SessionError');
          if (transkey_apiuse) {
  ...
  this.setUrl = function () {
    ...
    console.log("this.keyboardType==== ", this.keyboardType)
  ...
  this.setUrlUseBlob = function () {
    if (transkey_divType == 0) {
      ...
      if (this.keyboardType == 'numberMobile') {
        request.open(
          'GET',
          getUrl('getKey', this, 'single', this.allocationIndex) +
            '&talkBack=' +
            self.talkBack,
          true
        );
        ...
        request.onreadystatechange = function () {
          if (request.readyState == 4 && request.status == 200) {
            ...
            self.numberDiv.style.backgroundImage =
        ...
        request.onreadystatechange = function () {
          if (request.readyState == 4 && request.status == 200) {
            ...
            quertyImg.onload = function () {
              ...
              upper_request.setRequestHeader(
                'Content-type',
              ...
              upper_request.onreadystatechange = function () {
                if (upper_request.readyState == 4 && upper_request.status == 200) {
                  ...
                  self.upperDiv.style.backgroundImage =
                    "url('" + checkTag(upper_imageUrl) + "')";
                  self.upperDiv.style.filter =
                    "progid:DXImageTransform.Microsoft.AlphaImageLoader( src='" +
                    upper_imageUrl +
                    "', sizingMethod='scale')";
              ...
              special_request.setRequestHeader(
                'Content-type',
                'application/x-www-form-urlencoded'
              );
              special_request.onreadystatechange = function () {
                if (special_request.readyState == 4 && special_request.status == 200) {
                  ...
                  self.specialDiv.style.backgroundImage =
                    "url('" + checkTag(special_imageUrl) + "')";
                  self.specialDiv.style.filter =
                    "progid:DXImageTransform.Microsoft.AlphaImageLoader( src='" +
                    special_imageUrl +
                    "', sizingMethod='scale')";
            ...
            self.lowerDiv.style.backgroundImage =
    ...
    request.onreadystatechange = function () {
      if (request.readyState == 4 && request.status == 200) {
        ...
        if (request.responseText.indexOf('SessionError') > -1) {
          ...
          if (errCodes[1] == '1') {
            tk_alert('onreadystatechange SessionError');
            if (transkey_apiuse) {
              mtk.resetToken(transkey_apiurl);
              mtk.resetSessionKey(transkey_apiurl);
            } else {
  ...
  this.getInitTime = function () {
    ...
    if (transkey_apiuse) {
      request.open(
        'GET',
        transkey_apiurl + 'service/inittime?' + new Date().getTime() + tk_origin,
        false
      );
    } else {
      request.open(
    ...
    if (type == 'single') {
      ...
      for (var i = 1; 5 > i; i++) {
        for (var j = 0; 11 > j; j++) {
          try {
            ...
            console.log(
              '22[mTranskey Error] : backgroundImage error , (type=' + type + ')'
            );
          }
        }
      }
      ...
      for (var j = 0; k > j; j++) {
        try {
          ...
          console.log(
            '[mTranskey Error] : backgroundImage(5,0) error , (type=' + type + ')'
          );
        }
      }
    }
  ...
  this.setBgImageForDivType1UseBlob = function (type) {
    ...
    request.open(
      'GET',
      getUrl('getKey', this, type, this.allocationIndex) + '&talkBack=' + self.talkBack,
      true
    );
    if (useCORS) request.withCredentials = true;
          ...
          for (var i = 1; 5 > i; i++) {
            for (var j = 0; k > j; j++) {
              try {
                if (i == 4 && j == k - 1 && !useNoDummy) continue;
                if (useNoDummy == false && relocation == true && i == 4 && j == 1) {
                  self.div.childNodes[i].childNodes[
                    j
  ...
  function getUrlRestApi(op, o, keyType, allocationIndex) {
    if (!useSession) {
      ...
      //세션 사용시
      // op, o, keyType, allocationIndex
      // getUrlRestApi('service/key', this, type, this.allocationIndex);
      console.log('getUrlRestApi===', op, o, keyType, allocationIndex)
      return (
        transkey_apiurl +
        op +
        '?name=' +
...
function mTranskey() {
  ...
  this.cssText = [];
  this.cssText['qwertyMobile'] = [];
  this.cssText['numberMobile'] = [];
  this.transkeyUuid;
  this.clientWidth;
  ...
  this.getPublicKey = function (url) {
    ...
    if (transkey_apiuse)
      request.open(
        'GET',
        url + 'service/' + operation + '?TK_requestToken=' + TK_requestToken + tk_origin,
        useAsyncTranskey
      );
    ...
    try {
      if (transkey_apiuse) {
        console.log("transkey_apiuse request.body::: ", request.body)
        request.send();
      } else {
  ...
  this.generateSessionKey = function (url) {
    ...
    console.log("genSessionKey:::: ", genSessionKey)
    ...
    request.onreadystatechange = function () {
      if (request.readyState == 4 && request.status == 200) {
        ...
        if (request.responseXML) {
          ...
          console.log("===================responseXML1",  request.responseXML)

          console.log("===================responseXML", result)
          var res = null;
          var returns = 'return [';
          for (var i = 0; keyboardLayouts.length > i; i++) {
            ...
            if (node.tagName == 'script') {
              ...
              for (var j = 0; j < node.childNodes.length; j++) {
                if (node.childNodes[j].nodeValue.length > 10) {
                    // console.log(":::: node.childNodes[j].nodeValue::: ", node.childNodes[j])
                    console.log("useCSP:: ", useCSP)
                    // console.log("node1::: ", node.childNodes[j].nodeValue)
                    // console.log("node1::: ", node.childNodes[j].nodeValue.replace('//', ''))
                  if (useCSP) {
                    ...
                    console.log("returns::::",  node.childNodes[j].nodeValue.replace('//', '') + returns)
                    res = new Function(
                      Key + node.childNodes[j].nodeValue.replace('//', '') + returns
                    )();
                    console.log("2222")
            ...
            function promistSetKey(index) {
              return new Promise((resolve, reject) => {
                setTimeout(() => {
                  ...
                  if (useSession) {
                    transkey[inputs.item(index).id].setKeyType(
                      transkey[inputs.item(index).id].keyType
                    );
                    if (transkey_divType == 0) transkey[inputs.item(index).id].setUrl();
                  } else {
          ...
          if (useCSP) {
            ...
            for (var i = 0; i < transkey.objs.length; i++) {
              ...
              var isQwerty =
                document
                  .getElementById(transkey.objs[i])
                  .getAttribute('data-tk-kbdtype')
                  .indexOf('qwerty') > -1
                  ? true
                  : false;
              //input preview layout_close
              if (
                document.getElementById(transkey.objs[i]).getAttribute('data-tk-useinput')
              ) {
                if (document.addEventListener) {
                  document
                    .getElementById('input_label_close_' + transkey.objs[i])
                    .addEventListener('click', mtk.close);
                } else {
                  document
                    .getElementById('input_label_close_' + transkey.objs[i])
                    .attachEvent('onclick', mtk.close);
                }
              }
                ...
                (function (j, keypad, keypadChild) {
                  if (j == 0) {
                    if (keypad.childNodes[0].childNodes.length > 0) {
                      if (document.addEventListener) {
                        keypad.childNodes[0].childNodes[
                          keypad.childNodes[0].childNodes.length - 1
                        ].addEventListener(startEvent, mtk.close);
                      } else {
                        keypad.childNodes[0].childNodes[
                          keypad.childNodes[0].childNodes.length - 1
                        ].attachEvent('on' + startEvent, mtk.close);
                      }
                    }
                  } else {
                    ...
                    if (j < KeypadLength - 1) {
                      for (var k = 0; k < keypadChild.childNodes.length; k++) {
                        if (j == KeypadLength - 2 && k == 0 && isQwerty) {
                          if (document.addEventListener) {
                            keypadChild.childNodes[0].addEventListener(
                              startEvent,
                              function () {
                            ...
                            keypadChild.childNodes[0].attachEvent(
                              'on' + startEvent,
                              function () {
                                mtk.cap(event, keypadChild.childNodes[0]);
                              }
                        ...
                        if (
                          j == KeypadLength - 2 &&
                          k == keypadChild.childNodes.length - 1 &&
                          isQwerty
                        ) {
                          if (document.addEventListener) {
                            keypadChild.childNodes[
                              keypadChild.childNodes.length - 1
                            ].addEventListener(startEvent, function () {
                              mtk.del(
                                event,
                                keypadChild.childNodes[keypadChild.childNodes.length - 1]
                              );
                            });
                          } else {
                            keypadChild.childNodes[
                              keypadChild.childNodes.length - 1
                            ].attachEvent('on' + startEvent, function () {
                              mtk.del(
                                event,
                                keypadChild.childNodes[keypadChild.childNodes.length - 1]
                              );
                        ...
                        (function (k, index) {
                          if (document.addEventListener) {
                            keypadChild.childNodes[k].addEventListener(
                              startEvent,
                              function () {
                            ...
                            keypadChild.childNodes[k].attachEvent(
                              'on' + startEvent,
                              function () {
                                mtk.start(event, keypadChild.childNodes[k], index);
                              }
                      ...
                      if (isQwerty) {
                        ...
                        if (document.addEventListener) {
                          ...
                          if (relocation) {
                            //재배열
                            keypadChild.childNodes[1].addEventListener(
                              startEvent,
                              function () {
                          ...
                          if (useSpace) {
                            keypadChild.childNodes[
                              keypadChild.childNodes.length - 2
                            ].addEventListener(startEvent, function () {
                              mtk.start(
                                event,
                                keypadChild.childNodes[keypadChild.childNodes.length - 2],
                                41
                              );
                            });
                          }
                          ...
                          if (relocation) {
                            //재배열
                            keypadChild.childNodes[1].attachEvent(
                              'on' + startEvent,
                              function () {
                                mtk.relocate(event, keypadChild.childNodes[1]);
                              }
    ...
    try {
      ...
      tk_alert('77[mTranskey Error] : Cannot load TransKey. Network is not available.');
      return false;
    }
  };
  ...
  this.resetToken = function (url) {
    ...
    if (transkey_apiuse) {
      ...
      console.log("====", 'GET',
        url + 'service/token?' + new Date().getTime() + tk_origin,request.withCredentials)
    } else {
      request.open(
        'GET',
  ...
  this.getText = function (encrypted, ele) {
        ...
        if (request.responseText.indexOf('SessionError') > -1) {
          ...
          if (mtk.now.useInput) {
            ...
            if (mtk.now.useInputDiv.childElementCount > 4) {
              mtk.now.useInputDiv.childNodes[value.length * 2 + 1].value =
    ...
    try {
      ...
      tk_alert('00[mTranskey Error] : Cannot load TransKey. Network is not available.');
      return false;
    }
  };
  ...
  function SeedEnc(geo) {
    var iv = [
      0x4d, 0x6f, 0x62, 0x69, 0x6c, 0x65, 0x54, 0x72, 0x61, 0x6e, 0x73, 0x4b, 0x65, 0x79,
      0x31, 0x30
    ];
    var tsize = 64;
  ...
  function qwertyMobileLayoutForDIV_noInline(id, cssText) {
    ...
    if (document.getElementById(id).getAttribute('data-tk-useinput') == 'true') {
      ...
      layout +=
        '<a class="dv_' +
        cssText +
        '_disp_b" role="button" tabindex="0" aria-label="가상키보드 닫기"></a></div>';
    }
  ...
  function numberMobileNDLayoutForDIV(id, cssText) {
    //    var layout = '<div id="mtk_disp" class="dv_' + cssText + '_disp" style="height: 0px; display: none;">';
    var layout = '<div id="mtk_disp" class="dv_' + cssText + '_disp">';
    if (document.getElementById(id).getAttribute('data-tk-useinput') == 'true') {
      layout =
        '<div id="mtk_disp" style="height: 1px; border:0px; dislay: none;" role="button" tabindex="0" aria-label="가상키보드 영역 입니다"></div>';
    } else {
      layout +=
        '<div class="dv_' +
        cssText +
    ...
    layout +=
      ...
      '_div_a dv_' +
      cssText +
      '_div2_Height bio_auth"  ' +
      startEvent +
      //      '="mtk.del(event, this);"' +
      '="();"' +
      endEvent +
      //      '="balloonRemove();"  aria-label="삭제"><div class="dv_' +
      '="requestBioAuth();" aria-label=""><div class="dv_' +
      cssText +
      '_div2_3 dv_' +
      cssText +
  ...
  this.processTouchMove = function (x, y) {
    ...
    if (!mtk.now.gridsize) {
      ...
      mtk.now.gridsize =
        parseInt(mtk.now.buttons[1].pos.left) - parseInt(mtk.now.buttons[0].pos.left);
    }
...
mTranskey.prototype.setKeyboard = function (inputObj) {
  ...
  if (!useAsyncTranskey) {
    if (useSession) {
      ...
      if (transkey_divType == 0) transkey[inputObj.id].setUrl();
    } else {
...
mTranskey.prototype.onKeyboard = function (inputObj) {
  if (!useTalkBack && !clickDummy) {
    console.log(
      '현재 clickDummy의 상태는 false입니다. useTalkback 설정을 true로 변경해 주세요.'
    );
  }
  ...
  if (_useBioAuth !== true) return;
  const bioAuthElement = document.getElementsByClassName('bio_auth')[0];
  if (bioAuthElement.childElementCount !== 1) return;

  const child = bioAuthElement.children[0];
  child.style = `width: 100%; height: 100%; background-image: url("icon_faceid.svg"); background-size: 30px; background-repeat: no-repeat;`;
};
...
mTranskey.prototype.start = function (e, ele, index) {
  ...
  console.log("tk_Special_Mask_StartPos:::: ", mtk.now.tk_Special_Mask_StartPos)
...
mTranskey.prototype.setUseBioAuthentication = function (useBioAuth) {
  _useBioAuth = useBioAuth;
};
...
mTranskey.prototype.keyPress = function (e, ele, useFK) {
  ...
  try {
    ...
    if (useBalloon && mtk.now.keyboardType.indexOf('qwerty') > -1) {
      if (
        ele.parentElement.className.indexOf('end') < 0 &&
        ele.id != 'mtk_cp' &&
        ele.id != 'mtk_del'
      ) {
...
mTranskey.prototype.bioAuthCallBack = function () {};
...
mTranskey.prototype.setCapStyle = function () {
  ...
  if (transkey_divType == 1 && mtk.now.keyboardType == 'qwertyMobile') {
    if (mtk.now.keyType == 'upper') {
      mtk.now.div.childNodes[4].childNodes[0].classList.remove(
        'dv_' + cssName + '_div_a'
      );
...
mTranskey.prototype.reSizeForType1 = function (transkeyObj) {
  ...
  if (transkeyObj.keyboardType == 'numberMobile') {
    ...
    if (max_width != 0 && max_width >= 300 && this.clientWidth > max_width) {
      ...
      var edgeSize =
        this.clientWidth - (Math.ceil(ddivWidth * k) + (k + 1) + marginEdge * (k + 1));
    } else {
      var ddivWidth =
        Math.floor((this.clientWidth * widthRatio - (k + 1) - marginEdge * (k + 1)) / k) -
        divNum;
      var edgeSize =
        this.clientWidth / widthRatio -
        (Math.ceil(ddivWidth * k) + (k + 1) + marginEdge * (k + 1));
    }
    ...
    if (max_width != 0 && max_width >= 300 && this.clientWidth > max_width) {
      ...
      var ddivWidth =
        Math.floor((this.clientWidth * widthRatio - 12 - marginEdge * 12) / 11) -
        divKeypad;
      var edgeSize =
    ...
    if (max_width != 0 && max_width >= 300 && this.clientWidth > max_width) {
      ...
      var ddivWidth =
        ...
        divNum;
      var edgeSize =
        this.clientWidth / widthRatio - (ddivWidth * k + (k + 1) + marginEdge * (k + 1));
    }
    var edgePx = new Array(k);
    for (var i = 0; i < k; i++) {
      if (i < edgeSize) edgePx[i] = 1;
...
function getKeyArray(func) {
  ...
  var numberMobile = [];
  var key;
  console.log("getKeyArray:::: ", qwertyMobile )
